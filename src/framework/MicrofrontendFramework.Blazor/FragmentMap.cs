...
internal class FragmentMap
{
    ...
    private FragmentMap(IDictionary<string, ComponentCollection> dict)
    {
        _dict = dict;
    }
    ...
    {
        return _dict.TryGetValue(slotName, out var components) ? components : new ComponentCollection();
    }

    private static IEnumerable<string> GetSlotNames(Type component)
    {
        return component.GetCustomAttributes<FragmentAttribute>().Select(attr => attr.SlotName);
    }
}